...
class database {
    async init() {
        this.db = await new Promise((resolve) => {
            let request = indexedDB.open('database', 2);  // Увеличена версия с 1 до 2
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('servers')) {
                    db.createObjectStore('servers', { keyPath: "key" });
                }

                if (!db.objectStoreNames.contains('server-selected')) {
                    db.createObjectStore('server-selected', { keyPath: "key" });
                }
            }
