...
import { config, logger, changePanel, database, t } from './utils.js';
...
class Launcher {
    async init() {
        ...
        this.initFrame();
        gameConsole.init(); // Инициализируем консоль игры
        this.config = await config.GetConfig();
        console.log("Config loaded:", JSON.stringify(this.config, null, 2));
        console.log("Servers count:", this.config?.servers?.length || 0);
            ...
            for (let account of accounts) {
                ...
                if (account.meta.type === 'AZauth') {
                    ...
                    if (account.uuid === selectedAccount) {
                        ...
                        const skin_url = `${azauth}api/skin-api/avatars/${refresh.name}/?t=${timestamp}`;
    ...
    async initPreviewSkin() {
        ...
        const titleEl = document.querySelector('.player-skin-title');
        if (titleEl) {
            titleEl.innerHTML = `Скин ${account.name}`;
        }

        // Старый iframe удалён, теперь используется 3D вьювер в настройках
        // Этот метод оставлен для совместимости, но iframe больше не используется
    }
    ...
    updateMoney(account) {
        ...
        // Залишаємо порожнім, щоб не ламати виклики
    }
